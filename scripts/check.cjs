...
GlobalFonts.registerFromPath(path.join(root,'assets/caveat.ttf'),'Trazo');
...
function fixture({reduced=false,storageBlocked=false}={}){
 ...
 w.matchMedia=()=>({matches:reduced});w.requestAnimationFrame=()=>1;w.queueMicrotask=fn=>fn();w.HTMLCanvasElement.prototype.setPointerCapture=function(){};
